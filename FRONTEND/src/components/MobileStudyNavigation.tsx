...
interface MobileStudyNavigationProps {
  onClose: () => void;
}
...
export function MobileStudyNavigation({ onClose }: MobileStudyNavigationProps) {
  ...
  function selectDiscipline(slug: string) {
    ...
    const topic = selected?.topics[0];
    if (!selected || !topic) return;
    const subtopic = topic.subtopics[0];
    navigate(subtopic ? `/disciplina/${selected.slug}/topico/${topic.slug}/subtopico/${subtopic.slug}` : `/disciplina/${selected.slug}/topico/${topic.slug}`);
  }

  function selectTopic(slug: string) {
    const topic = discipline?.topics.find((item) => item.slug === slug);
    if (!discipline || !topic) return;
    const subtopic = topic.subtopics[0];
    navigate(subtopic ? `/disciplina/${discipline.slug}/topico/${topic.slug}/subtopico/${subtopic.slug}` : `/disciplina/${discipline.slug}/topico/${topic.slug}`);
  }

  function selectSubtopic(slug: string) {
    if (!discipline || !topic || !slug) return;
    navigate(`/disciplina/${discipline.slug}/topico/${topic.slug}/subtopico/${slug}`);
    onClose();
  }
...
export function MobileStudyRedirect() {
  ...
  useEffect(() => {
    ...
    const subtopic = topic?.subtopics[0];
    if (discipline && topic && subtopic) {
      navigate(`/disciplina/${discipline.slug}/topico/${topic.slug}/subtopico/${subtopic.slug}`, { replace: true });
    }
