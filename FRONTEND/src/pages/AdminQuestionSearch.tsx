...
type Editing =
  ...
  | { kind: "cloze"; id: number; subtopicId: number; textWithAnswers: string; isActive: boolean; isDifficult: boolean };
...
export function AdminQuestionSearch() {
  ...
  function startCloze(item: ClozeQuestion) { setEditing({ kind: "cloze", id: item.id, subtopicId: item.subtopicId, textWithAnswers: item.textWithAnswers, isActive: item.isActive, isDifficult: item.isDifficult }); }
  ...
  function editor() {
    ...
    return <form className="question-inline-editor" onSubmit={save}>
      ...
      {editing.kind === "cloze" && <><label><span>Texto com lacunas</span><textarea rows={5} value={editing.textWithAnswers} onChange={(event) => setEditing({ ...editing, textWithAnswers: event.target.value })} required /></label><label><span>Dificuldade</span><select value={editing.isDifficult ? "difficult" : "easy"} onChange={(event) => setEditing({ ...editing, isDifficult: event.target.value === "difficult" })}><option value="easy">Fácil</option><option value="difficult">Difícil</option></select></label></>}
  ...
  return <section className="page admin-tool-page question-search-page">
    ...
    {isLoading ? <div className="question-search-empty">Pesquisando questões...</div> : appliedSearch ? <div className="question-search-results">
      ...
      {kind === "cloze" && filteredClozes.map((item) => editing?.kind === kind && editing.id === item.id ? <article className="question-search-edit-row" key={item.id}>{editor()}</article> : <article className="question-search-row" key={item.id}><div><span>{context(item.subtopicId)}</span><p>{item.textWithAnswers}</p><small>{item.isDifficult ? "Difícil" : "Fácil"}</small></div><strong>{item.isActive ? "Ativa" : "Inativa"}</strong>{actions(kind, item.id, item.subtopicId, () => startCloze(item))}</article>)}
